...
import fondo from "../../assets.css/fondos/Fondo PRECEPTORIA.png"; // Asumiendo fondo de preceptoría
import { supabase } from "../../components.css/supabaseClient";
import logo from "../../assets.css/logos/Logo.png";

const MesasDeExamen = ({ goBack, goHome }) => {
  ...
  const initialFormState = {
    ...
    curso: "",
    division: "",
    condicion: "REGULAR", // Valor por defecto
    asignatura: "",
    inscriptos: 0,
    presentes: 0,
    ausentes: 0,
  ...
  const fetchAsignaturas = async () => {
    try {
      // Obtenemos las asignaturas únicas de la tabla de códigos para el select
      const { data, error } = await supabase
        .from('codigos')
        .select('asignatura')
        .neq('asignatura', null);
  ...
  // Manejo de cambios en el formulario y cálculos automáticos
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Cálculos automáticos si cambian los valores numéricos
      ...
      // Si cambia la condición a algo que no sea REGULAR, limpiamos asignatura si queremos forzar escritura,
      // o la dejamos. Aquí la lógica es: si es REGULAR usa select, sino input texto.
                  ...
                  {filteredData.map((item) => (
                    <tr 
                      ...
                      style={{ 
                        ...
                        backgroundColor: mode === "edit" && selectedId === item.id ? "#fffbe6" : "transparent"
                      }}
                    >
